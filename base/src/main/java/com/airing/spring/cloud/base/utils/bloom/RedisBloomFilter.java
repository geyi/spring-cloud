...
public final class RedisBloomFilter<T> implements Predicate<T>, Serializable {
    ...
    public interface Strategy extends Serializable {
        ...
        <T> boolean put(
                ...
                RedisBloomFilterStrategies.LockFreeBitArray bits);
     ...
     */
    private final RedisBloomFilterStrategies.LockFreeBitArray bits;
    ...
    private RedisBloomFilter(
            RedisBloomFilterStrategies.LockFreeBitArray bits, int numHashFunctions, Funnel<? super T> funnel,
            Strategy strategy, String filterName) {
        ...
        this.filterName = checkNotNull(filterName);
    }
    ...
    public RedisBloomFilter<T> copy() {
        return new RedisBloomFilter<T>(bits.copy(), numHashFunctions, funnel, strategy, filterName);
    }
     ...
     * RedisBloomFilter} with false positive probability 3%.
     *
     ...
     * @since 23.0
     */
    public static <T> Collector<T, ?, RedisBloomFilter<T>> toBloomFilter(
            Funnel<? super T> funnel, long expectedInsertions, String filterName) {
        return toBloomFilter(funnel, expectedInsertions, 0.03, filterName);
    }

    /**
     * Returns a {@code Collector} expecting the specified number of insertions, and yielding a {@link
     * RedisBloomFilter} with the specified expected false positive probability.
     *
     * <p>Note that if the {@code Collector} receives significantly more elements than specified, the
     * resulting {@code BloomFilter} will suffer a sharp deterioration of its false positive
     * probability.
     ...
     */
    public static <T> Collector<T, ?, RedisBloomFilter<T>> toBloomFilter(
            Funnel<? super T> funnel, long expectedInsertions, double fpp, String filterName) {
        checkNotNull(funnel);
        checkArgument(
        ...
        return Collector.of(
                () -> RedisBloomFilter.create(funnel, expectedInsertions, fpp, filterName),
                RedisBloomFilter::put,
                (bf1, bf2) -> {
                    bf1.putAll(bf2);
                    return bf1;
                },
    ...
    /**
     * Creates a {@link RedisBloomFilter} with the expected number of insertions and expected false
    ...
    public static <T> RedisBloomFilter<T> create(
        ...
        return create(funnel, expectedInsertions, fpp, Constant.DEFAULT_BLOOM_FILTER_NAME);
    }
    ...
    public static <T> RedisBloomFilter<T> create(
            Funnel<? super T> funnel, long expectedInsertions, double fpp, String filterName) {
        return create(funnel, expectedInsertions, fpp, RedisBloomFilterStrategies.MURMUR128_MITZ_64, filterName);
    }

    @VisibleForTesting
    static <T> RedisBloomFilter<T> create(
            Funnel<? super T> funnel, long expectedInsertions, double fpp, Strategy strategy, String filterName) {
        checkNotNull(funnel);
        checkArgument(
                expectedInsertions >= 0, "Expected insertions (%s) must be >= 0", expectedInsertions);
        checkArgument(fpp > 0.0, "False positive probability (%s) must be > 0.0", fpp);
        ...
        try {
            return new RedisBloomFilter<T>(new RedisBloomFilterStrategies.LockFreeBitArray(numBits, filterName),
                    numHashFunctions, funnel, strategy, filterName);
    ...
    /**
     * Creates a {@link RedisBloomFilter} with the expected number of insertions and a default expected
     ...
     * @return a {@code BloomFilter}
     */
    public static <T> RedisBloomFilter<T> create(Funnel<? super T> funnel, int expectedInsertions, String filterName) {
        return create(funnel, (long) expectedInsertions, filterName);
    }
     ...
     * @since 19.0
     */
    public static <T> RedisBloomFilter<T> create(Funnel<? super T> funnel, long expectedInsertions, String filterName) {
        return create(funnel, expectedInsertions, 0.03, filterName); // FYI, for 3%, we always get 5 hash functions
    }
    ...
    private static class SerialForm<T> implements Serializable {
        ...
        SerialForm(RedisBloomFilter<T> bf) {
            this.data = RedisBloomFilterStrategies.LockFreeBitArray.toPlainArray(bf.bits.data);
        ...
        Object readResolve() {
            return new RedisBloomFilter<T>(new RedisBloomFilterStrategies.LockFreeBitArray(data, filterName),
                    numHashFunctions,
                    funnel,
                    strategy,
                    filterName);
     ...
     */
    public static <T> RedisBloomFilter<T> readFrom(InputStream in, Funnel<? super T> funnel)
            throws IOException {
        throw new BusinessException(ExceptionEnum.NOT_SUPPORT_ERROR);
    }
}
